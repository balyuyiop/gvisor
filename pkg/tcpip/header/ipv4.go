...
func padIPv4OptionsLength(l uint8) uint8 {
	return (l + IPv4IHLStride - 1) & ^uint8(IPv4IHLStride-1)
}
...
// IPv4SerializableOptionWithPayload is a serializable IP option that defines a
// payload apart from just the option type.
type IPv4SerializableOptionWithPayload interface {
	// length returns the *payload* size of the option (not considering the type
	// and length fields).
	length() uint8

	// serializeInto serializes the payload of the receiver into the provided
	// byte buffer.
	//
	// serializeInto is not responsible for writing the option type or length
	// bytes, only the payload.
	//
...
func (s IPv4OptionsSerializer) Length() uint8 {
	...
	for _, opt := range s {
		total++
		if withPayload, ok := opt.(IPv4SerializableOptionWithPayload); ok {
...
var _ IPv4SerializableOptionWithPayload = (*IPv4SerializableRouterAlertOption)(nil)

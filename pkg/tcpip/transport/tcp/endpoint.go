...
func newEndpoint(s *stack.Stack, netProto tcpip.NetworkProtocolNumber, waiterQueue *waiter.Queue) *endpoint {
	...
	e.ops.SetSendBufferSize(DefaultSendBufferSize)

	var ss tcpip.TCPSendBufferSizeRangeOption
	if err := s.TransportProtocolOption(ProtocolNumber, &ss); err == nil {
		e.ops.SetSendBufferSize(ss.Default)
	}
...
func (e *endpoint) Readiness(mask waiter.EventMask) waiter.EventMask {
	...
	if e.EndpointState().connected() {
		...
		if (mask & waiter.EventOut) != 0 {
			e.sndBufMu.Lock()
			if e.sndClosed || e.sndBufUsed < e.ops.GetSendBufferSize() {
...
func (e *endpoint) isEndpointWritableLocked() (int, *tcpip.Error) {
	...
	avail := e.ops.GetSendBufferSize() - e.sndBufUsed
	if avail <= 0 {
		return 0, tcpip.ErrWouldBlock
	}
	return avail, nil
...
func (e *endpoint) OnSendBufferSizeOptionSet(sendBufferSize int) int {
...
func (e *endpoint) updateSndBufferUsage(v int) {
	sendBufferSize := e.ops.GetSendBufferSize()
	e.sndBufMu.Lock()
	notify := e.sndBufUsed >= int(sendBufferSize)>>1
	e.sndBufUsed -= v
...
func (e *endpoint) completeState() stack.TCPEndpointState {
	...
	e.sndBufMu.Lock()
	s.SndBufSize = e.ops.GetSendBufferSize()

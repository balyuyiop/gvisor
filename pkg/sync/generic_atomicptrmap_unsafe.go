...
// Package generic_atomicptrmap doesn't exist. This file must be instantiated using
// the go_template_instance rule in tools/go_generics/defs.bzl.
package generic_atomicptrmap
...
type apmShardLookupData struct {
	...
	mask  uintptr        // always (a power of 2) - 1; protected by rehaashMu/seq
}
...
type apmSlot struct {
	...
	// transition from empty to non-empty is irreversible for a given table;
...
retry:
	...
	for {
		...
		if slotVal == nil {
			...
			slotVal = atomic.LoadPointer(&slot.val)
			if slotVal == nil {
				// If inserting into this empty slot would make 2/3rds of the
				// table contain non-empty slots, rehash before insertion.
				if dirty, capacity := shard.dirty+1, mask+1; dirty*3 >= capacity*2 {
			...
			// Raced with another store; the slot is no longer empty.
			shard.dirtyMu.Unlock()
		}
		if slotVal == evacuated() {
			// Racing with rehashing.
...
func (shard *apmShard) rehash(oldSlots unsafe.Pointer) {
	...
	// currently smaller than shard.dirty, concurrent calls to shard.store()
	// that reuse existing slots can drive shard.count back up to a maximum of
	// shard.dirty.
	//
	// We rehash to a new table of the same size if count is 1/6th of the
	// current table's size or less (so the next rehashing will occur after the
	// insertion of 2/3-1/6=1/2 the table's size in new entries), and double
	// the new table's size otherwise.
	newSize := uintptr(8) // arbitrary initial size
	if oldSlots != nil {
		oldSize := shard.mask + 1
		newSize = oldSize
		if count := atomic.LoadUintptr(&shard.count) + 1; count*6 > oldSize {
			newSize *= 2
		}
	}
